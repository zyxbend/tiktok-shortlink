...
import com.pddbend.shortlink.project.common.convention.exception.ClientException;
...
import com.pddbend.shortlink.project.config.GotoDomainWhiteListConfiguration;
import com.pddbend.shortlink.project.dao.entity.*;
import com.pddbend.shortlink.project.dao.mapper.*;
...
import com.pddbend.shortlink.project.dto.resp.*;
import com.pddbend.shortlink.project.mq.producer.DelayShortLinkStatsProducer;
...
import static com.pddbend.shortlink.project.common.constant.RedisKeyConstant.*;
import static com.pddbend.shortlink.project.common.constant.ShortLinkConstant.AMAP_REMOTE_URL;
import static com.pddbend.shortlink.project.common.enums.ShortLinkErrorCodeEnum.SHORT_LINK_NOT_EXIST;
...
//        IPage<ShortLinkDO> resultPage = baseMapper.selectPage(requestParam, queryWrapper);
    ...
    public void updateShortLink(ShortLinkUpdateReqDTO requestParam) {
        ...
        if (Objects.equals(hasShortLinkDO.getGid(), requestParam.getGid())) {
            LambdaUpdateWrapper<ShortLinkDO> updateWrapper = Wrappers.lambdaUpdate(ShortLinkDO.class)
                    ...
                    .eq(ShortLinkDO::getDelFlag, 0)
                    .eq(ShortLinkDO::getEnableStatus, 0)
                    .set(Objects.equals(requestParam.getValidDateType(), VaildDataTypeEnum.PERMANENT.getType()), ShortLinkDO::getValidDate, null);
            ShortLinkDO shortLinkDO = ShortLinkDO.builder()
        ...
        if (!Objects.equals(hasShortLinkDO.getValidDateType(), requestParam.getValidDateType())
            ...
            if (hasShortLinkDO.getValidDate() != null && hasShortLinkDO.getValidDate().before(new Date())) {
                if (Objects.equals(requestParam.getValidDateType(), VaildDataTypeEnum.PERMANENT.getType()) || requestParam.getValidDate().after(new Date())) {
    ...
    public void restoreUrl(String shortUri, ServletRequest request, ServletResponse response) {
        ...
        boolean containsLink = shortUriCreateCachePenetrationBloomFilter.contains(fullShortUrl);
        if (!containsLink) {
            ((HttpServletResponse) response).sendRedirect("/page/notfound");
            return;
        }
        String gotoLinkIsNull = stringRedisTemplate.opsForValue().get(String.format(GOTO_IS_NULL_SHORT_LINK_KEY, fullShortUrl));
        if (StrUtil.isNotBlank(gotoLinkIsNull)) {
            ((HttpServletResponse) response).sendRedirect("/page/notfound");
            return;
        }
        ...
        try {
            ...
            // 当数据库中查询不到短链接或者短链接超过有效期时，返回短链接不存在（防止缓存穿透）
